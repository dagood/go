...
//
// If hostOS and hostArch are both nonempty and the OS/ARCH don't match, the build directory is
// treated as a cross-compiled build of Go from the host OS/ARCH to the detected OS/ARCH.
...
// directory. Optionally takes version, os, and arch, and detects their values from the environment
// and runtime if empty string.
func DefaultBuildOutputPath(source, version, os, arch string) string {
	if version == "" {
		version = getBuildID()
	}
	if os == "" {
	...
	archiveName := fmt.Sprintf("go.%v.%v-%v%v", version, os, arch, ext)
